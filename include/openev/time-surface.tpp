...
void TimeSurface_<T>::render() {
  const T ON = this->imageValues_[TimeSurface_<T>::value::ON];
  const T OFF = this->imageValues_[TimeSurface_<T>::value::OFF];
  const T RESET = this->imageValues_[TimeSurface_<T>::value::RESET];
  ...
  } else if constexpr(cv::DataType<T>::channels > 1) {
...
void TimeSurface_<T>::clear_() {
  this->setTo(this->imageValues_[EventImage_<T>::value::RESET]);
  timestamp.setTo(0, cv::noArray());
  polarity.setTo(0, cv::noArray());
}

template <typename T>
void TimeSurface_<T>::insert_(const Event &e) {
